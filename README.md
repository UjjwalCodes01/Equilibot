...
> **Live Demo:** [equilibot.vercel.app](https://equilibot.vercel.app) · **Agent API:** [equilibot.onrender.com](https://equilibot.onrender.com)
...
- Fetches real-time BNB/USD prices from [Pyth Network](https://pyth.network/) Hermes API
...
1. **Off-chain Simulation** — Every intent is dry-run against a local fork before submission
2. **On-chain SwapGuard** — Deployed smart contract enforces router allowlists, token allowlists, max slippage (300 bps), and oracle freshness
3. **Gnosis Safe Module** — The agent can only call pre-approved, policy-compliant transactions
4. **Execution Mode Ladder** — `observe → simulate → canary → active` — must be manually escalated by the operator
...
**1. AI Thought Narrator (Nexus page)**
→ Click the **✨ AI Insight** link under any log entry. Gemini narrates the agent's reasoning in plain English. Results are cached — no duplicate API calls.

**2. AI Audit Explainer (Audits page)**
→ Click any audit entry to expand it, then click **✨ Analyze with AI**. Gemini writes a 3–4 sentence post-mortem of the decision.

**3. AI Strategy Builder (Studio page)**
→ Type into the **✨ AI Strategy Builder** box and press Enter. Example prompt:
```
